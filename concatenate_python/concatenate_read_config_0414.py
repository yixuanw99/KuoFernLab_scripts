...
print("coding list =",coding_list)
print("noncoding list =",noncoding_list)

alignment_FileName = TaxaName + ".fasta"
presence_table_FileName = TaxaName + ".csv"
parition_file_FileName = TaxaName + "_partition" + ".txt"
parition12_file_FileName = TaxaName + "_partition_codon12" + ".txt"
parition3_file_FileName = TaxaName + "_partition_codon3" + ".txt"
	...
	if file.split('.')[name_length-1] == 'fas': #file type filelsit and taxalist
		...
		for line in open(fasta_file_name, "r"):
			if line.startswith('>'):
				line = line.rstrip()
				fa_ID = line.replace('>','')
				taxa_raw_list.append(fa_ID.split('-')[0])
...
for locus in loci_list:
	locus_file_name = import_align_dir + locus + ".fas" #file type
	#print(locus)
	...
	for line in open(locus_file_name,"r").readlines():
		line = line.rstrip()
		if line.startswith('>'):
			fa_ID.append(line.replace('>','').split('-')[0])
			fa_Num = fa_Num + 1
			fa_Seq.append("")
		else:
