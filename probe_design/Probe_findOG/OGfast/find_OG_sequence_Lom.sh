...
do
	...
	do
		...
		if [[ ${filesize} -gt 2 ]];then
		  ...
		  grep -A 1 -w -f pattern_file_Lom.txt --no-group-separator /home2/yxwu/findOG/cds_with_intron/${sample_list[j]}.fas >> OG_Lomariopsis/${OG_list[$(($k-2))]}.fna
		else
		  echo "-----------"
		fi
